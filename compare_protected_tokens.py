...
def _strip_tag_name_and_closing(tag_text: str) -> str:
...
def _split_semantic_tag(tag_text: str) -> Tuple[str, str]:
    inner = tag_text[1:-2]
    identifier, label = inner.split(":", 1)
    return identifier, label


def _compare_inventories(
    source_inventories: Dict[str, List[str]],
    candidate_inventories: Dict[str, List[str]],
...
def _compare_ordered_spans(
    ...
    for index in range(paired_count):
        ...
        if source_category in PLACEHOLDER_CATEGORIES and candidate_category in PLACEHOLDER_CATEGORIES:
            if source_span["text"] != candidate_span["text"]:
                errors.append(
                    _make_error(
                        "PLACEHOLDER_CHANGED",
                        f"Placeholder changed at span index {index}: "
                        f"{source_span['text']} -> {candidate_span['text']}.",
        ...
        if source_category == "percent_token_with_literal_percent" and candidate_category == "percent_token":
            errors.append(
                _make_error(
                    ...
                    f"Literal trailing percent sequence changed at span index {index}: "
                    f"{source_span['text']} -> {candidate_span['text']}.",
        ...
        if source_category != candidate_category:
            errors.append(
                _make_error(
                    ...
                    f"Protected token order/category changed at span index {index}: "
                    f"{source_category} -> {candidate_category}.",
        ...
        if source_category in PLACEHOLDER_CATEGORIES and source_span["text"] != candidate_span["text"]:
            errors.append(
                _make_error(
                    "PLACEHOLDER_CHANGED",
                    f"Placeholder changed at span index {index}: "
        ...
        if source_category in {"font_open_tag", "panel_tag"}:
            source_name = _strip_tag_name_and_closing(str(source_span["text"]))
            candidate_name = _strip_tag_name_and_closing(str(candidate_span["text"]))
            if source_name != candidate_name:
                errors.append(
                    _make_error(
                        "TAG_SHAPE_CHANGED",
                        f"Tag name changed at span index {index}: {source_name} -> {candidate_name}.",
                        category=str(source_category),
                        source_span=str(source_span["text"]),
                        candidate_span=str(candidate_span["text"]),
                    )
            ...
            source_attrs = _extract_attributes(str(source_span["text"]))
            candidate_attrs = _extract_attributes(str(candidate_span["text"]))
        ...
        if source_category == "semantic_tag":
            ...
            if source_identifier != candidate_identifier:
                errors.append(
                    _make_error(
                        "TAG_SHAPE_CHANGED",
                        f"Semantic tag identifier changed at span index {index}: "
                        f"{source_identifier} -> {candidate_identifier}.",
            ...
            if source_label != candidate_label:
                errors.append(
                    _make_error(
                        ...
                        f"Semantic tag label changed at span index {index}: "
                        f"{source_label} -> {candidate_label}.",
                        category="semantic_tag",
                        source_span=str(source_span["text"]),
                        candidate_span=str(candidate_span["text"]),
                    )
...
def compare_protected_tokens(source: str, candidate: str) -> Dict[str, object]:
    ...
    return {
        ...
        "errors": errors,
        "source_tokens": {
            "spans": source_result["spans"],
            "inventories": source_result["inventories"],
            "warnings": source_result["warnings"],
        },
        "candidate_tokens": {
            "spans": candidate_result["spans"],
            "inventories": candidate_result["inventories"],
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Compare protected localization tokens between source and candidate.")
    parser.add_argument("source", help="Source localized value.")
    parser.add_argument("candidate", help="Candidate localized value.")
